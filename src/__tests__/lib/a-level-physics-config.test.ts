...
import { EXAM_CONFIG } from "@/lib/a-level-physics/config";

describe("A Level Physics Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM107");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("a-level-physics");
  });
